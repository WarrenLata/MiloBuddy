...
def create_agent(user_name: str | None = None, description: str = "Main chat agent. Discuss with the user and decides which tools to use to answer the user's questions.") -> Agent:
...
class MainAgentRunner(Runner):
    ...
    """
    def __init__(self, user_id: str = "user", user_name: str | None = None, session_id: str | None = None):
    ...
    async def create(cls, user_id: str = "user", user_name: str | None = None):
        ...
        instance.session = await instance.session_service.create_session(user_id=user_id, app_name="MAIN_CHAT", session_id=session_id)
            ...
            async for event in self.runner.run_async(
                ...
                if  event.is_final_response():
                    if (
                        ...
                        logger.warning(
                            f"==> Final Agent Response: [No text content in final event] : {event}! Session ID: {self.session.id}")
                        return final_response_text
        except Exception as e:
            logger.error(f"Error occurred while calling agent: {e}! Session ID: {self.session.id}")
